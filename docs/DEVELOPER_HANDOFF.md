...
Cloudflare Pages deploys the static export bundle `out/` generated by `npm run build`. Host-level routing, root `/` -> `/tr/` redirects and security/cache headers are defined in `public/_redirects` and `public/_headers`. Supabase is deployed independently; its migrations and Edge Functions are not deployed by the Cloudflare Pages build. Google Mail, Turnstile and analytics are external services.
...
Never commit real environment files, admin passwords, service-role keys, database credentials, private keys, Google Mail keys or Turnstile secrets. Transfer administrator credentials and organization/project access for Supabase, Cloudflare, registrar/DNS, Google Workspace, GA/GTM and Search Console through a secure channel outside the repository. Confirm least privilege and remove departing users after handoff.
