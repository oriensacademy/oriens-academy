...
export function QuickContactLead() {
  ...
  function submit(event: React.FormEvent) {
    ...
    startTransition(async () => {
      const result = await submitContact({ fullName: isTr ? "Hızlı İletişim Talebi" : "Quick Contact Request", email: email.trim().toLowerCase(), message: isTr ? "Web sitesi hızlı iletişim kutusundan geri dönüş talebi." : "Callback requested through the website quick-contact box.", locale, privacyConsent: true, turnstileToken: token, source: "quick_contact" });
      if (result.success) {
        setDone(true);
        sessionStorage.setItem(DISMISS_KEY, "1");
      } else {
  ...
  return <AnimatePresence>{visible && <motion.aside initial={reducedMotion ? false : { opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 12 }} transition={{ duration: reducedMotion ? 0 : .24 }} className="fixed right-6 bottom-6 z-[45] hidden w-[340px] overflow-hidden rounded-2xl border border-[#DDE4DC] bg-white p-5 pb-6 shadow-[0_22px_60px_rgba(16,39,27,.18)] lg:block" aria-label={isTr ? "Hızlı iletişim" : "Quick contact"}>
    ...
    {done ? <div role="status" className="py-4 text-center"><span className="mx-auto flex size-10 items-center justify-center rounded-full bg-[#E9EFE9] text-[#10271B]"><Check className="size-5" /></span><p className="mt-3 font-heading text-xl text-[#10271B]">{isTr ? "Talebiniz alındı" : "Request received"}</p><button type="button" onClick={dismiss} className="mt-4 text-xs font-bold text-[#68756C] underline underline-offset-4">{isTr ? "Kapat" : "Close"}</button></div> : <form onSubmit={submit}>
      <p className="pr-8 text-xs font-bold tracking-[.18em] text-[#819586] uppercase">{isTr ? "Size ulaşalım" : "Let us contact you"}</p>
      <h2 className="mt-2 font-heading text-2xl text-[#10271B]">{isTr ? "Akademik rotanızı konuşalım." : "Let's discuss your academic route."}</h2>
      <p className="mt-2 text-xs leading-relaxed text-[#68756C]">{isTr ? "E-posta adresinizi bırakın, ekibimiz sizinle iletişime geçsin." : "Leave your email and our team will get in touch."}</p>
      <div className="mt-4 flex gap-2"><input data-locale-field="quick-contact-email" type="email" value={email} onChange={(event) => setEmail(event.target.value)} aria-label={isTr ? "E-posta adresi" : "Email address"} placeholder={isTr ? "E-posta adresiniz" : "Your email"} className="min-w-0 flex-1 rounded-lg border border-[#DDE4DC] px-3 py-2.5 text-sm outline-none focus:border-[#819586]" /><button disabled={pending} className="flex size-11 shrink-0 items-center justify-center rounded-lg bg-[#10271B] text-white disabled:opacity-50" aria-label={isTr ? "Bana ulaşın" : "Contact me"}><Send className="size-4" /></button></div>
      <label className="mt-3 flex min-w-0 items-start gap-1.5 text-[10px] leading-[1.3] text-[#68756C]"><input data-locale-field="quick-contact-consent" type="checkbox" checked={consent} onChange={(event) => setConsent(event.target.checked)} className="mt-0.5 shrink-0" /><span className="min-w-0 break-words">{isTr ? "İletişim için verilerimin işlenmesini kabul ediyorum." : "I consent to processing my details for contact."} <Link href={`/${locale}/privacy`} className="font-bold underline">{isTr ? "Gizlilik" : "Privacy"}</Link></span></label>
