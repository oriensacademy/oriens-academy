...
function Card({ item, duplicate = false }: { item: MarqueeTestimonial; duplicate?: boolean }) {
  return (
    <article tabIndex={duplicate ? -1 : 0} aria-hidden={duplicate || undefined} className="flex h-[19rem] w-[min(82vw,25rem)] shrink-0 flex-col rounded-2xl border border-border bg-white/95 p-5 shadow-xs outline-none focus-visible:ring-2 focus-visible:ring-primary/35 sm:h-[18rem] sm:w-[23rem] sm:p-6">
      <Quote className="size-5 text-warm-accent" aria-hidden="true" />
      <blockquote className="mt-4 min-h-0 flex-1 overflow-hidden"><p className="line-clamp-6 whitespace-pre-wrap text-sm leading-7 text-ink/82">“{item.text}”</p></blockquote>
      <footer className="mt-4 flex shrink-0 items-center gap-3 border-t border-border/70 pt-4">
