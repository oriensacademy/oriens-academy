    ...
    <div className="fixed inset-0 z-[120] flex items-center justify-center p-3 sm:p-6">
      ...
      <section role="dialog" aria-modal="true" aria-labelledby="refund-dialog-title" className="relative z-10 max-h-[92vh] w-full max-w-2xl overflow-y-auto rounded-3xl border border-border bg-white p-5 shadow-2xl sm:p-7">
        ...
        {context ? <>
          ...
          {context.refunds.length ? <div className="mt-5 rounded-2xl border border-border p-4"><h3 className="text-xs font-semibold text-ink">İade Geçmişi / Refund History</h3><div className="mt-3 space-y-2">{context.refunds.map((item) => <div key={item.id} className="grid gap-1 border-t border-border pt-2 text-[11px] text-muted-foreground sm:grid-cols-3"><span>{money(Number(item.amount))} · -{item.lessons} ders</span><span>{item.status} · {item.provider_reference}</span><span>{new Date(item.finalized_at || item.created_at).toLocaleString("tr-TR")} · {item.reason} · Admin: {item.admin_actor}</span></div>)}</div></div> : null}
