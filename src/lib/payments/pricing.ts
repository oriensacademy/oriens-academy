...
export function calculateAuthoritativeTotal(params: {
  ...
  if (coupon && subtotalKurus > 0) {
    ...
    // Check minimum order amount requirement if defined
    ...
    if (minOrderKurus > 0 && subtotalKurus < minOrderKurus) {
      ...
      // Find eligible items for package-targeted coupons
      ...
      if (eligibleKurus > 0) {
        if (coupon.discount_type === "percentage") {
          // Percentage discount: (eligibleKurus * discount_value) / 100 in integer kuruş
