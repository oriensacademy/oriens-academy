...
export class ImperialProgramAdapter implements ProgramSourceAdapter {
  name = "ImperialProgramAdapter";

  canHandle(url: string, domain: string): boolean {
    return domain.includes("imperial.ac.uk");
  }
  ...
  async discoverProgramLinks(sourceUrl: string, html: string, _ctx: AdapterContext): Promise<DiscoveredProgramLink[]> {
    ...
    while ((match = anchorRegex.exec(html)) !== null) {
      ...
      if (href.includes("/study/courses/") || href.includes("/study/ug/") || href.includes("/study/pg/")) {
        try {
          const resolved = new URL(href, baseUrl).toString();
          const can = canonicalizeUrl(resolved);
          if (!seen.has(can) && !resolved.endsWith("/courses/") && !resolved.endsWith("/study/courses")) {
            seen.add(can);
            discovered.push({
              url: resolved,
              title: text,
  ...
  async extractProgram(html: string, pageUrl: string, ctx: AdapterContext): Promise<ExtractedProgramRecord | null> {
    ...
    const isPostgrad = pageUrl.includes("/pg/") || pageUrl.includes("/postgraduate/");
    const degreeTitle = extractDegreeTitle(name) || extractDegreeTitle(html) || (isPostgrad ? "MSc" : "BSc");
