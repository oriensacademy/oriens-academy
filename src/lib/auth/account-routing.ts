...
import { ensureTrailingSlash, localizedPath, unifiedLoginPath, SITE_URL } from "@/lib/routes";

export function safeReturnPath(value: string | null): string | null {
  if (!value || !value.startsWith("/") || value.startsWith("//") || value.includes("\\")) return null;
  try {
    const parsed = new URL(value, SITE_URL);
    return parsed.origin === SITE_URL ? ensureTrailingSlash(`${parsed.pathname}${parsed.search}${parsed.hash}`) : null;
  } catch {
    return null;
  }
}
