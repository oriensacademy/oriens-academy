...
const topicSequence = ["topic-a", "topic-b", "topic-c", "topic-a", "topic-b", "topic-c", "topic-a", "topic-b"] as const;
const correctSequence: AnswerId[] = ["a", "b", "c", "d", "a", "b", "c", "d"];
...
function createQuestion(exam: ExamCode, index: number): ExamTestQuestion {
  ...
  const topic = topicSequence[index];
  return {
    ...
    correctAnswer: correctSequence[index],
    explanation: {
...
export const examTests: Record<ExamCode, ExamTest> = Object.fromEntries(
  examCodes.map((exam) => [exam, { exam, questions: Array.from({ length: 8 }, (_, index) => createQuestion(exam, index)) }])
...
export function calculateTestResult(test: ExamTest, answers: Record<string, AnswerId>, locale: Locale): TestResult {
  ...
  return {
    ...
    accuracy: Math.round((correct / test.questions.length) * 100),
