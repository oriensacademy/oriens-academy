...
Deno.serve(async (req: Request) => {
  ...
  const { data, error } = await admin
    .from("bookings")
    .select("id,full_name,email,phone,exam_code,custom_exam,appointment_subject,event_type,notes,status,student_user_id,live_meeting_url,availability_slots(starts_at,ends_at)")
    .eq("id", bookingId)
    .single();

  if (error || !data) {
    return buildJsonResponse({ error_code: "BOOKING_NOT_FOUND" }, 404, req);
  }
  ...
  if (isUpdate && slot?.starts_at) {
    ...
    await dispatchBookingEmails(admin, {
      ...
      email: data.email,
      phone: data.phone,
      supportType: data.event_type === "lesson" ? "exam_preparation" : "university_support",
