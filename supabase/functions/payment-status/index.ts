...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { data, error } = await admin.from("payment_transactions").select("public_reference,package_id,amount,currency,status,payment_method,provider,created_at,paid_at").eq("public_reference", reference).eq("status_token_hash", tokenHash).maybeSingle();
    if (error || !data) return buildJsonResponse({ error_code: "PAYMENT_NOT_FOUND", message: "Payment could not be verified." }, 404, req);
    return buildJsonResponse({ success: true, payment: { reference: data.public_reference, packageId: data.package_id, amount: data.amount, currency: data.currency, status: data.status, paymentMethod: data.payment_method, provider: data.provider, createdAt: data.created_at, paidAt: data.paid_at } }, 200, req);
  } catch {
